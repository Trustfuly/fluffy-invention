...
function update_script() {
  # IMPORTANT: Disable the helper script error traps to prevent accidental deletion
  trap - EXIT
  trap - ERR
  ...
  # Find Container IDs
  local UPD_CTIDS=$(grep -lE "^tags:.*yopass" /etc/pve/lxc/*.conf | xargs -n1 basename | cut -d. -f1 || true)
