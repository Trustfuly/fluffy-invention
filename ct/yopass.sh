...
function update_script() {
  ...
  for CTID in $UPD_CTIDS; do
    ...
    pct exec "$CTID" -- bash -c "
        ...
        # Clean the directory but keep the directory itself
        ...
        # SMART DEPLOY: Find where 'index.html' actually is inside the repo
        # If your files are in 'public/', we copy from there.
        if [ -d \"/tmp/yopass_update/public\" ]; then
            cp -a /tmp/yopass_update/public/. /var/www/yopass/
        else
            # Fallback: copy everything if 'public' folder doesn't exist
            cp -a /tmp/yopass_update/. /var/www/yopass/
        fi
        
        # Clean up and set strict permissions
        ...
        # Clear Nginx cache (if any) and restart
        rm -rf /var/lib/nginx/proxy/*
        systemctl start yopass
        systemctl restart nginx

        # Verification of the asset folder
        if [ -d \"/var/www/yopass/assets\" ]; then
            echo -e \"  \${G}[OK]\${NC} Frontend assets successfully deployed.\"
            ls -F /var/www/yopass/assets | head -n 5
        else
            echo -e \"  \${R}[ERROR]\${NC} Assets folder missing! Check your GitHub repo structure.\"
        fi
